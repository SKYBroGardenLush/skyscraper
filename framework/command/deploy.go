...
import (
	...
	"github.com/SKYBroGardenLush/skyscraper/framework/containerService/service/ssh"
...
func deployBuildBackend(c *cobra.Command, deployFolder string) error {
	...
	// 复制.env文件
	if utils.Exists(filepath.Join(appService.BaseFolder(), ".env")) {
		if err := utils.CopyFile(filepath.Join(appService.BaseFolder(), ".env"), filepath.Join(deployFolder, ".env")); err != nil {
			return err
		}
	}
	...
	if !utils.Exists(deployConfigFolder) {
		if err := os.MkdirAll(deployConfigFolder, os.ModePerm); err != nil {
			return err
		}
	}
	if err := utils.CopyFolder(filepath.Join(appService.ConfigFolder(), env), deployConfigFolder); err != nil {
		return err
	}
...
func deployBuildFrontend(c *cobra.Command, deployFolder string) error {
	...
	//if err := utils.CopyFolder(buildFolder, frontendFolder); err != nil {
	//	return err
	//}
	return nil
}
