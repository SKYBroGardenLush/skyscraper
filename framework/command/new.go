...
var newCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		currentPath := utils.GetExecDirectory()
		...
		}
		{
			...
			prompt := &survey.Input{
				Message: "请输入版本名称(参考 https://github.com/SKYBroGardenLush/skyscraper/releases，默认为最新版本)：",
			}
			...
			if version != "" {
				...
				if err != nil || release == nil {
					fmt.Println("版本不存在，创建应用失败，请参考 https://github.com/SKYBroGardenLush/skyscraper/releases")
					return nil
				}
			}
			if version == "" {
		...
		fmt.Println("skyscraper框架版本：", release.GetTagName())
		...
		// 获取folder下的SKYBroGardenLush-skyscraper-xxx相关解压目录
